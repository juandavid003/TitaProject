...
class _ArBrushPageIosState extends State<ArBrushPageIos> {
  ...
  void _addPlane(ARKitPlaneAnchor anchor) async {
    aRKitAnchor = anchor;
    anchorId = anchor.identifier;

    if (!play) {
      _changeAnimation("idle", "models.scnassets/Tita_Anim_Idle.dae");
    }
  }

  // Function to change animation and model
  void _changeAnimation(String animationName, String modelPath) {
    if (aRKitAnchor == null) return;
    ...
    // Create new node
    final newNode = ARKitReferenceNode(
        name: 'tita-$animationName',
        url: modelPath,
        position: vector.Vector3(0, 0, 0),
        scale: vector.Vector3(0.01, 0.01, 0.01));

    // Add new node
    arkitController.add(newNode, parentNodeName: aRKitAnchor!.nodeName);
    currentNode = newNode;
  }
    ...
    if (_allChildrens.isEmpty) {
      ...
      if (play) {
        ...
        if (_start == _totalTime && play) {
          ...
          bool shouldProceed = await _startBrushing(context);

          if (shouldProceed && aRKitAnchor != null) {
            // Change to brushing animation
            _changeAnimation(
                "brushing", "models.scnassets/Tita_Anim_Cepillado.dae");

            if (player.playing == false) {
              await player.setAsset(
                  'assets/audio/TitaAnimRender_Cepillado_Todo_Aud2.mp3');

              player.play();
            }

            // After the full brushing time, change to the new animation
            _timer = Timer(Duration(seconds: 175), () {
              if (play && _currentAnimation == "brushing") {
                _completeAnimation();
              }
            });
          }
        } else if (_currentAnimation == "new" ||
  ...
  void _completeAnimation() async {
    // if (!play || aRKitAnchor == null) return;

    // Change to new animation
    _changeAnimation("new", "models.scnassets/new.dae");

    // After new animation completes, go to finish screen
    _timer = Timer(Duration(seconds: 44), () {
  ...
  Future<bool> _startBrushing(BuildContext context) async {
                            ...
                            ? () {
                                ...
                                WidgetsBinding.instance
                                  ...
                                  if (_hasConfirmedSelection &&
                                      !_controllerCount.isStarted.value) {
                                    _controllerCount.start();
                                  }
                                });
      ...
      (Timer timer) => setState(
        () {
          if (_start < 1) {
            ...
            // If we're still in brushing animation at timer end, move to next animation
            if (_currentAnimation == "brushing") {
              _completeAnimation();
            }
