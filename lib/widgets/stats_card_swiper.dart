...
class _StatsCardSwiperState extends State<StatsCardSwiper> {
  ...
  Widget _body(PersonModel item, BuildContext context,
      List<BrushingHistoryModel> brushingHistory, BbCashModel bbCash) {
    return Padding(
  ...
  Row _headerPerson(
      PersonModel item, BuildContext context, BbCashModel bbCash) {
...
// }
  ...
  int _cantPerDay(List<BrushingHistoryModel> brushingHistoryWeek, int weekDay) {
    ...
    int adjustedDay = (weekDay == 0) ? 6 : weekDay - 1;

    DateTime start = startOfWeek.add(Duration(days: adjustedDay));
    DateTime end = start.add(const Duration(days: 1));

    List<BrushingHistoryModel> brushingHistoryDay =
        brushingHistoryWeek.where((element) {
