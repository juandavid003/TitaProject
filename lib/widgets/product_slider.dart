...
class ProductSlider extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      // Eliminar altura fija para permitir que se adapte al contenedor padre
      child: GridView.builder(
        padding: const EdgeInsets.symmetric(vertical: 8),
        gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
          crossAxisCount: 2, // Dos tarjetas por fila
          crossAxisSpacing: 8, // Espaciado horizontal entre tarjetas
          mainAxisSpacing: 8, // Espaciado vertical entre tarjetas
          childAspectRatio: 0.75, // Ajuste de proporción ancho-alto
        ),
...
class _ProductCard extends StatelessWidget {
  ...
  Widget _imagesProducts(ProductModel item, BuildContext context) {
    // Verificar que imageUrl no sea nula ni vacía
    if (item.imageUrl == null || item.imageUrl!.isEmpty) {
      // Mostrar una imagen de placeholder si la URL es nula o vacía
      return Container(
        width: double.infinity,
        height: Utils.size(context).height * 0.2,
    ...
    return CachedNetworkImage(
      ...
      width: double.infinity,
      height: Utils.size(context).height * 0.2,
      // Agregar un placeholder y un manejador de errores
