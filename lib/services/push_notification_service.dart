...
// P8 - KeyID: VYZH37GGZ9 -  B6SC9DMN69
...
static Future _onMessageOpenApp(RemoteMessage message) async {
  final caseType = message.data['caseType'];
  _messageStream.add(caseType);
  goToScreenIfNeeded(caseType);
}



static void goToScreenIfNeeded(String caseType) {
  switch (caseType) {
    case "BrushingTime":
      navigatorPlus.key.currentState?.push(
        MaterialPageRoute(builder: (_) => BrushScreen()),
      );
      break;
    case "TimeToSchedule":
      navigatorPlus.key.currentState?.push(
        MaterialPageRoute(builder: (_) => AppoitmentScreen()),
      );
      break;
    // case "¡Hey! ¿Qué tal tu experiencia? Cuentanos como te fue?":
    //   navigatorPlus.key.currentState?.push(
  ...
  static Future initializeApp() async {
    ...
    if (settings.authorizationStatus == AuthorizationStatus.authorized ||
      ...
      token = null;
      print(
          'Las notificaciones están deshabilitadas. Token establecido en null');
    }
  ...
  static Future<void> setupUser() async {
    while (Utils.globalFirebaseUser == null ||
      ...
      await Future.delayed(
          Duration(milliseconds: 100)); // Espera breve para evitar bloqueos
    }
  ...
  static Future<void> _checkAndUpdateUser(
      String uid, String? token, String cellphone) async {
    ...
    await userDoc.set(
      {"token": token, "cellphone": cellphone},
  ...
  static Future<void> _upadteOrCreateUserOnOdontofyApi(
      String uid, String? token, String cellphone) async {
    final response = await http.post(
      Uri.https(
          'odontofy.org', '/OdontoFyWebApi/api/Lead/InsertOrUpdateByPhone'),
      ...
      body:
          jsonEncode(<String, dynamic>{"Phone": cellphone, "TokenApp": token}),
    );
        ...
        children: [
          ...
          Align(
            ...
            child: GestureDetector(
              onTap: () async {
                if (_expanded) {
                  await PushNotificationService._onMessageOpenApp(widget.message);
                  widget.onClose();
                } else {
                  setState(() {
                    _expanded = !_expanded;
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (_expanded &&
                        widget.message.notification?.android?.imageUrl != null)
                      Padding(
