...
class ClinicsService {
  ...
  Future<List<ClinicsModel>> get() async {
    ...
    List<ClinicsModel> result =
        Clinics.fromJsonMap(querySnapshot.docs).clinicList;
  ...
  Future<List<ClinicsModel>> clinics(
      int limit, ClinicsModel? lastClinic) async {
    String code = (await storage.read(key: 'countryCode'))!;
    DocumentSnapshot? lastClinicDocument;
    ...
    if (lastClinic != null) {
      ...
      lastClinicDocument = findDocuments.docs[0];
    }

    QuerySnapshot querySnapshot;
    if (lastClinicDocument != null) {
      querySnapshot = await FirebaseFirestore.instance
          .collection("clinics")
          .orderBy('name')
          .startAfterDocument(lastClinicDocument)
          .limit(limit)
          .get();
    } else {
      querySnapshot = await FirebaseFirestore.instance
    ...
    final clinics = Clinics.fromJsonMap(querySnapshot.docs);
