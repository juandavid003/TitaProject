  ...
  static Future<String> loadImage(String fileName) async {
    try {
      ...
      if (paths.length > 0 && !paths[1].contains('null')) {
        // Implementar un mecanismo de caché para evitar múltiples solicitudes a la misma URL
        SharedPreferences prefs = await SharedPreferences.getInstance();
        String cachedUrl = prefs.getString('cache_$fileName') ?? '';

        if (cachedUrl.isNotEmpty) {
          return cachedUrl;
        }

        // Si no está en caché, obtener la URL y guardarla
        ...
        // Guardar en caché
        await prefs.setString('cache_$fileName', downloadUrl);
        return downloadUrl;
      } else {
        return "";
      }
    } catch (e) {
      print('Error al cargar la imagen $fileName: $e');
      return "";
    }
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        var jsonResponse = jsonDecode(
            utf8.decode(response.bodyBytes)); // Decodificación UTF-8 explícita
